...
import { getBookingStatusColor } from "@/utils/getMovieStatus";
...
const BookingHistory = () => {
  ...
  const { isPending: isDownloading } = useDownloadTicketMutation();
        ...
        <Stack gap="md">
          {bookings.map((booking: UserBookingType) => {
            ...
            const showDateTime = dayjs(
              `${booking.schedule.showDate} ${booking.schedule.showTime}`,
            );
            const isCancellable =
              booking.status === "confirmed" &&
              today.isBefore(showDateTime.subtract(2, "hour"));
            return (
              <Card
                key={booking.id}
                  ...
                  <Grid>
                    <Grid.Col span={6}>
                      <Stack gap="xs">
                        <Group gap="xs">
                          <IconMapPin size={16} />
                          <Text size="sm">{booking.theatre.name}</Text>
                        </Group>
                        <Text size="xs" c="dimmed" ml={22}>
                          {booking.theatre.location}, {booking.theatre.region},{" "}
                        ...
                        <Group gap="xs">
                          ...
                          <Text size="sm">
                            {dayjs(booking.schedule.showDate).format(
                              "MMM DD, YYYY",
                            )}{" "}
                            at{" "}
                            {dayjs(
                              `2000-01-01 ${booking.schedule.showTime}`,
                            ).format("h:mm A")}
                          </Text>
                        </Group>
                        ...
                        <Group gap="xs">
                          ...
                          <Text size="sm">
                            ...
                            {booking.schedule.subtitle &&
                              ` (Subtitle: ${booking.schedule.subtitle})`}
                          </Text>
                        </Group>
                      </Stack>
                    </Grid.Col>
